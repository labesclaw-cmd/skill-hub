...
// ── 設定 ──────────────────────────────────────────────────────────
// Option A: set DISCORD_BOT_TOKEN env var in the launchd plist
// Option B: place token in ~/.claude/discord_config.sh as:
//   DISCORD_BOT_TOKEN="your-token-here"
const TOKEN = (() => {
    if (process.env.DISCORD_BOT_TOKEN) return process.env.DISCORD_BOT_TOKEN;
    const cfgPath = path.join(process.env.HOME, '.claude/discord_config.sh');
    if (fs.existsSync(cfgPath)) {
        const m = fs.readFileSync(cfgPath, 'utf8').match(/DISCORD_BOT_TOKEN="([^"]+)"/);
        if (m) return m[1];
    }
    console.error('DISCORD_BOT_TOKEN not set. Use env var or ~/.claude/discord_config.sh');
    process.exit(1);
})();
const OWNER_ID   = process.env.DISCORD_OWNER_ID || '';  // your Discord user ID (leave blank to allow all)
